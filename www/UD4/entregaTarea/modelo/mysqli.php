...
function conectaTareas()
{
    ...
    return conecta($servername, $username, $password, $dbname);
}
...
function creaDB()
{
    ...
    try {
        ...
        else
        {
            ...
            $sqlCheck = "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = '$dbname'";
...
function createTablaUsuarios()
{
    try {
        ...
        else
        {
            ...
            $sql = 'CREATE TABLE `usuarios` (`id` INT NOT NULL AUTO_INCREMENT , `username` VARCHAR(50) NOT NULL , `rol` INT NOT NULL DEFAULT 0 , `nombre` VARCHAR(50) NOT NULL , `apellidos` VARCHAR(100) NOT NULL , `contrasena` VARCHAR(100) NOT NULL , PRIMARY KEY (`id`)) ';
            if ($conexion->query($sql))
            {
